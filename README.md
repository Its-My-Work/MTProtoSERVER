...
| **Управление пользователями** | Добавление, удаление, блокировка, копирование ссылки |
| **Управление прокси** | Создание, удаление, включение/выключение прокси серверов |
| **Статистика** | Графики трафика по пользователям |
| **Настройки** | Просмотр конфигурации, генерация API токена |
...
- 👥 **Пользователи** — управление доступом, копирование ссылок для подключения
- 🌐 **Прокси** — создание и управление прокси серверами
- 📈 **Статистика** — графики трафика
- ⚙️ **Настройки** — конфигурация, генерация API токена
- 🔧 **Диагностика** — проверка системы

### API

MTProtoSERVER предоставляет REST API для программного управления.

**Генерация токена:**
- Перейдите в **Настройки** → **API токен**
...
```bash
curl -X POST http://ВАШ_IP:8080/api/users/add \
  -H "Authorization: Bearer ВАШ_ТОКЕН" \
  -F "label=новый_клиент" \
  -F "proxy_id=1" \
  -F "expires=2024-12-31T23:59"
```

**Основные endpoints:**
- `POST /api/users/add` — добавить пользователя
- `POST /api/users/{id}/toggle` — включить/выключить пользователя
- `POST /api/users/{id}/delete` — удалить пользователя
- `POST /api/proxies/add` — добавить прокси
- `POST /api/proxies/{id}/toggle` — включить/выключить прокси
- `POST /api/proxies/{id}/delete` — удалить прокси
...
│
├── scripts/
│   ├── add-proxy.sh              # Добавление прокси
│   ├── remove-proxy.sh           # Удаление прокси
