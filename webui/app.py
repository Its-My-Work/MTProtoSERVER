...
def generate_faketls_secret(domain: str) -> str:
    """
    Генерирует корректный FakeTLS-секрет формата:
    ee + 16 байт ключа + hex(domain),
    используя mtg. Не пытается собирать секрет руками.
    """
    try:
        result = subprocess.run(
            ['docker', 'run', '--rm', 'nineseconds/mtg:2',
             'generate-secret', '--hex', domain],
        ...
        # Жёсткий fallback: только временно, без dom, чтобы не ломать формат
        # 16 байт ключа, без faketls-домена → обычный MTProto-секрет.
        return sec.token_hex(16)
...
def update_mtg_config():
    """
    Обновляет config.toml MTG с всеми активными секретами и портами из proxies.json.
    """
    pd = load_json(os.path.join(DATA_DIR, 'proxies.json'))
    proxies = pd.get('proxies', [])

    # Собираем уникальные порты
    unique_ports = list(set(p['port'] for p in proxies if p.get('enabled', True)))
    unique_ports.sort()

    # Собираем все активные секреты: base прокси + клиенты
    clients = get_clients()['clients']
    all_secrets = []
    for proxy in proxies:
        if proxy.get('enabled', True):
            all_secrets.append(proxy['secret'])
            proxy_clients = [c['secret'] for c in clients if c.get('proxy_id') == proxy['id'] and c.get('enabled', True)]
            all_secrets.extend(proxy_clients)

    # Путь к config.toml
    config_path = '/app/data/proxy.toml'
    ...
    if os.path.exists(config_path):
        ...
        config = {
            "bind-to": [],
            "fake_tls_domain": "yandex.ru",
            "prefer_ip": "prefer-ipv4",
            "secrets": {},
            "anti_replay": {"enabled": True, "max_size": 16384},
            "timeouts": {"inactivity": 300, "keepalive": 60},
            "stats": {"enabled": True, "stats_file": "/data/stats.json"}
        }

    # Обновляем bind-to
    config["bind-to"] = [f"0.0.0.0:{port}" for port in unique_ports]

    # Обновляем секреты: генерируем dict с ключами user1, user2, ...
    secrets_dict = {}
    for i, secret in enumerate(all_secrets, start=1):
        secrets_dict[f"user{i}"] = secret
    config["secrets"] = secrets_dict
...
async def add_client(request: Request):
    try:
        ...
        pid_str = form.get('proxy_id', '0')
        pid = 0 if pid_str == 'undefined' else int(pid_str or 0)
        if pid == 0:
            return JSONResponse({'status':'error','message':'Выберите прокси'})
        tlg_str = form.get('traffic_limit_gb', '0')
        tlg = 0.0 if tlg_str == 'undefined' else float(tlg_str or 0)
        dl_str = form.get('device_limit', '0')
        dl = 0 if dl_str == 'undefined' else int(dl_str or 0)
        ed_str = form.get('expiry_days', '0')
        ed = 0 if ed_str == 'undefined' else int(ed_str or 0)
        s = get_settings(); cd = get_clients(); cl = cd.get('clients',[]); nid2 = cd.get('next_id',1)

        # Получаем данные прокси
        pd = load_json(os.path.join(DATA_DIR, 'proxies.json'))
        proxy = next((p for p in pd.get('proxies', []) if p['id'] == pid), None)
        if not proxy:
            return JSONResponse({'status':'error','message':'Прокси не найден'})

        port = proxy['port']
        dom = proxy['domain']
        node_ip = s.get('proxy_ip','0.0.0.0')
        secret = generate_faketls_secret(dom)

        exp = (datetime.now()+timedelta(days=ed)).strftime('%Y-%m-%d') if ed>0 else ''
        cl.append({'id':nid2,'label':label,'proxy_id':pid,'port':port,'domain':dom,'secret':secret,'enabled':True,'created_at':datetime.now().strftime('%Y-%m-%d %H:%M:%S'),'traffic_limit_gb':tlg,'device_limit':dl,'expiry_date':exp,'auto_reset':form.get('auto_reset','never'),'rx_bytes':0,'tx_bytes':0,'unique_ips':0,'connections':0,'history':[]})
        cd['clients']=cl; cd['next_id']=nid2+1; save_clients(cd)

        # Обновляем config MTG с новыми секретами
        update_mtg_config()

        return JSONResponse({'status':'ok','secret':secret,'link':proxy_link(node_ip,port,secret),'port':port,'client_id':nid2,'node_ip':node_ip})
...
async def delete_client(cid:int):
    ...
    for c in cd.get('clients',[]):
        if c['id']==cid:
            proxy_id = c.get('proxy_id', 0)
            break
    cd['clients']=[c for c in cd.get('clients',[]) if c['id']!=cid]; save_clients(cd)
    if proxy_id is not None:
        update_mtg_config()
    return JSONResponse({'status':'ok'})
...
async def rotate_client(cid:int):
    ...
    for c in cd.get('clients',[]):
        if c['id']==cid:
            dom = c.get('domain','cloudflare.com')
            c['secret'] = generate_faketls_secret(dom)
            proxy_id = c.get('proxy_id', 0)
            break
    save_clients(cd)
    if proxy_id is not None:
        update_mtg_config()
    return JSONResponse({'status':'ok','secret':c['secret'],'link':proxy_link(node_ip,c['port'],c['secret'])})
...
async def add_node(request: Request):
    ...
    port = int(form.get('port',9876) or 9876)
    ns.append({'id':nid,'name':form.get('name','node'),'ip':form.get('ip',''),'port':port,'country':form.get('country','🌍'),'token':form.get('token',''),'auth_type':form.get('auth_type','token'),'ssh_user':form.get('ssh_user',''),'ssh_pass':form.get('ssh_pass',''),'ssh_key':form.get('ssh_key',''),'enabled':True,'created_at':datetime.now().strftime('%Y-%m-%d %H:%M:%S'),'last_ping':'','status':'unknown'})
    nd['nodes']=ns; nd['next_id']=nid+1; save_nodes(nd)
    # Добавляем ufw правило для порта ноды
...
async def create_mtproto(request: Request):
    ...
    save_settings(s)
    # Update MTG config with new proxy
    update_mtg_config()
